...
for_plot <-
  ...
  rename(Sex = sex, Quintiles = color_quintiles)
...
ggplot(for_plot,
  ...
  labs(
    title = "BHRC data PCA of 1st and 8th quintiles (ADHD)",
    subtitle = glue("N = {nrow(for_plot)}"),
    caption = "27-02-2024_cass_BHRC_QuintileADHDInvestigation.R"
  ) +
  scale_shape_manual(values = c("Female" = 24, "Male" = 22)) +
  scale_color_manual(values = c("3th" = "#387cb4", "4th" = "#ff9100", "other" = "#ececec")) +
  scale_fill_manual(values = c("3th" = "#387cb4", "4th" = "#ff9100",  "other" = "#ececec")) +
  theme_publish() +
  theme(
...
p2 <- for_plot %>%
    ...
    theme_minimal() +
    labs(
        title = "BHRC ADHD Quintiles description",
        subtitle = glue("N = {nrow(for_plot)}"),
        caption = "27-02-2024_cass_BHRC_QuintileADHDInvestigation.R"
    ) +
    theme(
        ...
        legend.title = element_text(size = 15, face = "bold"),
        legend.title.position = "top",
        plot.title = element_text(size = 35, face = "bold", hjust = 1.1),
...
p3 <- for_plot %>%
    ...
    rename(State = popID) %>%
    group_by(State, Quintiles) %>%
    summarise(N = n()) %>%
    group_by(Quintiles) %>%
    mutate(Proportion = N / sum(N)) %>%
    ggplot(aes(Quintiles, Proportion, fill = State)) +
    geom_bar(
        position = "fill",
        stat = "identity",
        alpha = 0.95,
